...
# 临时修改：将DepthWiseSeparableConv2d改为DepthWiseSeparableConv2d
from .utils import Conv2d, DepthWiseSeparableConv2d
...
class GE(nn.Module):
    def __init__(self, in_ch, out_ch=None, stride=1, expansion=6):
        ...
        if stride != 1:
            self.conv2 = nn.Sequential(
                DepthWiseSeparableConv2d(hidden_state, hidden_state, 3, stride, padding=1),
                nn.BatchNorm2d(hidden_state),
                DepthWiseSeparableConv2d(hidden_state, hidden_state, 3, 1, padding=1),
                nn.BatchNorm2d(hidden_state)
            )
        else:
            self.conv2 = nn.Sequential(
        ...
        if stride != 1:
            self.skip = nn.Sequential(
                DepthWiseSeparableConv2d(in_ch, in_ch, 3, stride=stride, padding=1),
...
class BGA(nn.Module):
    def __init__(self, dch, sch):
        ...
        self.dpath = nn.Sequential(
            DepthWiseSeparableConv2d(dch, dch, 3, 1, 1),
            nn.BatchNorm2d(dch),
        ...
        self.spath = nn.Sequential(
            DepthWiseSeparableConv2d(sch, sch, 3, 1, 1),
            nn.BatchNorm2d(sch),
